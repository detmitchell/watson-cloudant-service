...
Cloudant.prototype.find = function (query, callback) {
...
//Lists all db docs
Cloudant.prototype.listAll = function (callback) {
	db.find({ "selector": { "_id": { "$ne": 0 } } }, function (err, response) {
		if (err)
			callback(err);
		else
			callback(null, response);
...
Cloudant.prototype.bulkInsert = function (obj, callback) {
	db.bulk(obj, function (err, response) {
		if (err)
			callback(err);
		else
			callback(null, response);
